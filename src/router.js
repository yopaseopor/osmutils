...
// Add router menu to the map
    ...
    const steps = [
        { id: 'start', label: 'Set start point', icon: 'play', active: true },
        { id: 'via', label: 'Add via point', icon: 'dot-circle', active: false },
        { id: 'end', label: 'Set destination', icon: 'stop', active: false },
        { id: 'route', label: 'Show route', icon: 'route', active: false }
    ];
    ...
    function renderSteps() {
        ...
        steps.forEach((step, index) => {
            const stepElement = $('<div>')
                .addClass('router-step')
                .addClass(step.active ? 'active' : '')
                .addClass(index < currentStep ? 'completed' : '')
                .html(`<i class="fa fa-${step.icon}"></i> ${step.label}`)
                .on('click', () => goToStep(index));
                
            routerMenu.append(stepElement);
            
            // Add connector between steps
            if (index < steps.length - 1) {
                routerMenu.append($('<span>').addClass('router-step-connector').html('&rarr;'));
            }
        });
        
        // Add action buttons
        const actions = $('<div>').addClass('router-actions');
        ...
        if (currentStep < steps.length - 1) {
            $('<button>')
                .addClass('router-btn primary')
                .text('Next')
                .on('click', () => goToStep(currentStep + 1))
                .appendTo(actions);
        } else {
            $('<button>')
                .addClass('router-btn danger')
                .html('<i class="fa fa-times"></i> Clear')
                .on('click', clearRoute)
                .appendTo(actions);
        }
        
        routerMenu.append(actions);
    }
    ...
    function goToStep(stepIndex) {
        ...
        if (stepIndex === 2 && !startPlace) {
            alert('Please set a start point first');
            return;
        }
        
        if (stepIndex === 3 && (!startPlace || !endPlace)) {
    ...
    // Update map interaction based on current step
    ...
    // Clear the route and reset the UI
    function clearRoute() {
    ...
    let endMarker = null;
    let viaMarker = null;
    // Handle map clicks based on current step
    const clickHandler = function(evt) {
        ...
        if (currentStep === 0) {
            ...
            if (currentStep < steps.length - 1) {
                goToStep(currentStep + 1);
                // Calculate the route after a short delay to allow UI to update
                setTimeout(calculateRoute, 100);
            }
        }
    };
    
    // Calculate the route based on current points
    function calculateRoute() {
        if (!startPlace || !endPlace) return;
        
        // Get the current profile
        const profile = $('.profile-select').val() || 'car';
        ...
        fetch(url)
            ...
            .then(data => {
                ...
                steps[3].label = `${distance} km • ${duration} min`;
                renderSteps();
